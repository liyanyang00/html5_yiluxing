        ...
        <div class="col-md-10 col-xs-10 right">
            ...
            <div class="buttongroup">
                <div class="col-md-6 col-xs-6 left">
                    <form action="<?php echo U('Admin/Indent/search');?>" class="navbar-form" role="search" >
                ...
                <div class="col-md-6 col-xs-6 left">
                       <form action="<?php echo U('Admin/Indent/query');?>" class="navbar-form" role="search" style="margin-left:-50px;">
                        订单生成日期：
                        <input type="date" value="" name="date1">
                        至
                        <input type="date" value="" name="date2">
                        <button type="submit" class="button1" name="">查询</button>
                    </form>
                </div>
            </div>
            <div class="tablegroup">
                <table class="table">
                    ...
                    <?php if(is_array($indent)): $i = 0; $__LIST__ = $indent;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><tr data-toggle="modal" data-target="#mymodal-data" onclick="postIt(<?php echo ($d["ind_id"]); ?>)">
...
<script type="text/javascript">	
		...
		function postIt (res){	
			...
			function(data,status){	
				...
				$('#detail p span').eq(2).text("<?php echo ($result['user_name']); ?>");
				$('#detail p span').eq(3).text(<?php echo ($result['user_telephone']); ?>);
					return false;				
			});
