            ...
            <div class="tablegroup">
                <table class="table">
                    ...
                    <?php if(is_array($usertab)): $i = 0; $__LIST__ = $usertab;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><tr data-toggle="modal" data-target="#mymodal-data"onclick="postIt(<?php echo ($d["user_id"]); ?>)">
                          ...
                          <?php
 $where ='user_id = '.$d['user_id']; $result = M("usertab")->where($where)->find(); ?>
	...
	function postIt (res){	
			...
			function(data,status){	
				...
				for (var i = array.length - 1; i >= 0; i--) {
					$("#detail").append("<p class='p2 p4'><img src='/easygo/Public/Admin/images/img1.jpg' width='160' height='100'><span class='p44'>"+array[i]['inf_type']+"</span><span>"+array[i]['inf_carnumber']+"</span>"+array[i]['inf_province']+"/"+array[i]['inf_city']+"</p>");
				};
